...
class MayaSessionCollector(HookBaseClass):
    ...
    def check_name_duplacate(self):

      current_engine = sgtk.platform.current_engine()
      context = current_engine.context.entity
      context_step = current_engine.context.step
      nameg = context["name"] + "_"
      nameh = nameg.replace(" ", "_")
      namef = nameh.replace("-", "_")
      typef = context["type"]
      stepf = context_step["name"]
    ...
    def _collect_cameras(self, parent_item):
      ...
      context_step = current_engine.context.step
      stepf = context_step["name"]
      if stepf == "Animation" or stepf == "Layout" :
...
# BIBI ---------------------------------------------------------------------------------------------------------------------
    ...
    def _collect_meshes(self, parent_item):
      ...
      if stepf == "Shading" :
        ...
        # for any mesh.
        for object in cmds.ls(dag=True, type="mesh"):
        ...
        mesh_item.properties["object"] = allobj
...
# TITI ---------------------------------------------------------------------------------------------------------------------
    ...
    def _collect_session_geometry(self, parent_item):
        ...
        context_step = current_engine.context.step
        stepf = context_step["name"]
        if stepf == "Animation" or stepf == "Layout" or stepf == "Model" or stepf == "RigImport":
